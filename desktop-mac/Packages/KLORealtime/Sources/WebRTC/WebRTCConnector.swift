...
extension WebRTCConnector: LKRTCDataChannelDelegate {
	public func dataChannel(_: LKRTCDataChannel, didReceiveMessageWith buffer: LKRTCDataBuffer) {
		// Surface every server event to the unified log so we can debug
		// voice mode without attaching a debugger. The library used
		// print() for failures only, and even print() doesn't reach
		// os_log for sandboxed apps.
		let raw = String(data: buffer.data, encoding: .utf8) ?? "<binary>"
		NSLog("KLO Realtime DC: %@", String(raw.prefix(300)))

		// Broadcast the raw payload so klo's RealtimeBridge can react to
		// function-call events directly. The library's `Conversation
		// .entries` observable doesn't reliably surface function_call
		// items (verified empirically — response.function_call_arguments
		// .done fires server-side but entries stays empty), so we route
		// dispatch off the raw stream instead. Posting on NotificationCenter
		// keeps the swift-realtime-openai package independent of klo —
		// no klo-specific code in the library, just a generic
		// "here's what the server said" broadcast.
		NotificationCenter.default.post(
			name: Notification.Name("KLORealtimeServerEventRaw"),
			object: nil,
			userInfo: ["data": buffer.data]
		)

		do { try stream.yield(decoder.decode(ServerEvent.self, from: buffer.data)) }
		catch {
			// Both lines must be NSLog: sandboxed macOS apps drop print()
			// output, so swift `print` is invisible in production. NSLog
			// lands in unified log and is visible to `log show`.
			NSLog("KLO Realtime DC: decode FAILED — %@", error.localizedDescription)
			let raw = String(data: buffer.data, encoding: .utf8) ?? "<invalid utf8>"
			NSLog("KLO Realtime DC: raw payload that failed to decode — %@", raw)
