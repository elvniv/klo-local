		...
		task = Task.detached { [weak self] in
			...
			do {
				for try await event in self.client.events {
					// NSLog (not print) so errors land in the unified log
					// even from sandboxed app context. print() output is
					// dropped for App Store / signed app distributions.
					do { try await self.handleEvent(event) }
					catch { NSLog("KLO Realtime Conversation: handleEvent error — %@", String(describing: error)) }

					guard !Task.isCancelled else { break }
				}
			} catch {
				NSLog("KLO Realtime Conversation: task loop exited with error — %@", String(describing: error))
			}
		}
	}
		...
		switch event {
			...
			case let .responseFunctionCallArgumentsDone(_, _, itemId, _, _, arguments):
				updateEvent(id: itemId) { functionCall in
					...
					// klo patch: upstream never transitions
					// Item.FunctionCall.status to .completed; observers
					// polling entries for "this function call is done"
					// have no signal otherwise. The Done event is
					// definitionally the completion signal — mirror it
					// into status so client code can rely on it.
